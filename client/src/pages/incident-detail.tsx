	...
	return (
		<div className="mx-auto max-w-3xl">
			...
			<dl className="mt-6 grid grid-cols-2 gap-4 rounded-xl border border-slate-200 bg-white p-6 text-sm">
				...
				<Field label="Reported by" value={incident.reporter.fullName} />
				<Field label="Assignee" value={incident.assignee?.fullName ?? 'Unassigned'} />
			...
			{isAdmin && (
				<div className="mt-6 rounded-xl border border-slate-200 bg-white p-6">
					<h2 className="mb-4 text-sm font-semibold text-slate-900">Admin controls</h2>
					<div className="grid grid-cols-3 gap-4">
						<Control label="Status" value={incident.status} disabled={update.isPending} onChange={(value) => update.mutate({ id, payload: { status: value as typeof incident.status } })} options={STATUSES} />
						<Control label="Priority" value={incident.priority} disabled={update.isPending} onChange={(value) => update.mutate({ id, payload: { priority: value as typeof incident.priority } })} options={PRIORITIES} />
					...
					{update.isError && <p className="mt-3 text-sm text-red-500">Update failed — reverted.</p>}
				</div>
			)}
					...
					<ul className="space-y-2">
						{incident.auditLogs.map((log) => (
							<li key={log.id} className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600">
								<span className="font-medium text-slate-900">{log.actor.fullName}</span> changed <span className="font-medium">{log.field}</span> from{' '}
								<span className="text-slate-500">{log.oldValue ?? 'none'}</span> to <span className="text-slate-700">{log.newValue ?? 'none'}</span>
