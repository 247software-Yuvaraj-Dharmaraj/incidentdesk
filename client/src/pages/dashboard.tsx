...
const CARDS: { key: 'total' | 'OPEN' | 'IN_PROGRESS' | 'RESOLVED'; labelKey: string; accent: string }[] = [
	{ key: 'total', labelKey: 'dashboard.total', accent: 'text-slate-900' },
	{ key: 'OPEN', labelKey: 'dashboard.open', accent: 'text-blue-600' },
	{ key: 'IN_PROGRESS', labelKey: 'dashboard.inProgress', accent: 'text-amber-600' },
	{ key: 'RESOLVED', labelKey: 'dashboard.resolved', accent: 'text-green-600' },
];
	...
	return (
		<div>
			...
			<p className="mt-1 text-sm text-slate-500">
				{t('dashboard.welcome', { name: user?.fullName })} · <span className="font-medium text-slate-700">{user?.role}</span>
			</p>

			<div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
				{CARDS.map((card) => (
					<div key={card.key} className="rounded-xl border border-slate-200 bg-white p-5">
						<p className="text-xs uppercase tracking-wide text-slate-400">{t(card.labelKey)}</p>
